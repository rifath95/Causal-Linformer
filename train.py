...
os.makedirs("stuff", exist_ok=True)  # [Codex comment] Ensure the plot destination exists before saving the image.
...
plot_path = os.path.join("stuff", f"training_loss_{attention_type}_{plot_timestamp}.png")  # [Codex comment] Save every attention run under a unique filename without overwriting earlier plots.
...
plt.savefig(plot_path, dpi=300, bbox_inches="tight")  # [Codex comment] Save a high-resolution labeled loss plot in the stuff folder.
